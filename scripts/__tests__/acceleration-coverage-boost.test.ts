import { parseExpression, simplify, exprToString } from '../acceleration/symbolic-engine';
import { mergeResults, weightedMerge, mergeByMajority } from '../acceleration/result-merger';
import { OLLAMA_ADAPTER, LOCAL_MOCK_ADAPTER } from '../acceleration/local-model-adapter';
import { OPENAI_ADAPTER, ANTHROPIC_ADAPTER, GOOGLE_ADAPTER } from '../acceleration/remote-model-adapter';
import { summary, median, correlation, linearRegression, quartiles } from '../acceleration/stats-engine';
import { evaluateFormula, registerFormula, listFormulas, getFormula } from '../acceleration/formula-registry';
import { runAllBenchmarks, runBenchmark } from '../acceleration/benchmark';
import { reachConsensus, createConsensusGroup, mockConsensus } from '../acceleration/consensus';
import { shouldInvalidate, invalidateByAgeMap, invalidateByPrefixMap } from '../acceleration/cache-invalidation';
import { evaluatePolicies, addPolicy, clearPolicies, getPolicies } from '../acceleration/policy-engine';
import { validateInput, validateOutput } from '../acceleration/guardrails';
import { estimateBudget, mergeBudgets, budgetToString } from '../acceleration/budget';
import { estimateTokens, estimateTimeMs, estimateCostUsd, estimateTotal } from '../acceleration/estimator';
import { rankProviders, selectProvider } from '../acceleration/provider-router';
import { mean, stddev, variance, sum, clamp, lerp, min, max, roundTo } from '../acceleration/numerical-engine';
import { force, kineticEnergy, potentialEnergy, power, density, momentum, acceleration } from '../acceleration/physics-engine';
...
import { optimizeMode, suggestOptimizations, canRunLocally } from '../acceleration/optimizer';
import { isEnabled, enableFlag, disableFlag, listFlags, initializeFlags } from '../acceleration/feature-flags';
import { logAudit, getAuditLog, clearAuditLog, totalCost, totalTokens, successRate, exportAuditLog } from '../acceleration/audit-log';
import { compressSmart, compressToTarget, compressTrivial, compressRemoveComments, compressRemoveWhitespace, compressKeepStructure } from '../acceleration/context-compressor';
import { tokenizeMath, evaluateSimpleMath, extractNumbers, isNumeric } from '../acceleration/math-parser';
import { estimateTokensFromText, estimateTokensFromMessages, estimateTokensFromFiles, estimateOutputTokens } from '../acceleration/token-estimator';
import { countChars, countWords, estimateTokens as tokenMeterEstimate, measureCost, formatTokenReport } from '../acceleration/token-meter';
import { getCostEntry, calculateCost, listModels, cheapestModel } from '../acceleration/cost-model';
import { getLatency, estimateLatencyMs } from '../acceleration/latency-model';
