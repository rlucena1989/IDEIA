...
function runNpmAudit(): ThreatReport['npmAudit'] {
  try {
    ...
    const vulnerabilities: Record<string, NpmVulnerability> = data.vulnerabilities ?? {};
    const advisories: Record<string, NpmAdvisory> = data.advisories ?? {};
    ...
    return {
      ...
      advisories: Object.values(advisories).slice(0, 10).map(a => `${a.cve || a.github_advisory_id}: ${a.title}`),
    };
  } catch {
    ...
    try {
      const data = JSON.parse(stderr) as NpmAuditData;
      const vulnerabilities: Record<string, NpmVulnerability> = data.vulnerabilities ?? {};
      return {
        critical: Object.values(vulnerabilities).filter(v => v.severity === 'critical').length,
        high: Object.values(vulnerabilities).filter(v => v.severity === 'high').length,
