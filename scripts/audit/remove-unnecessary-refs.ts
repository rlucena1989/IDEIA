...
function removeUnnecessaryReferences(packagePath: string): number {
  ...
  try {
    ...
    for (const ref of tsconfig.references) {
      ...
      if (allDeps[depName]) {
        ...
        console.log('Removing reference from', packageName, ':', ref.path);
        removedCount++;
      }
    }
...
for (const pkg of packages) {
  ...
  totalRemoved += removeUnnecessaryReferences(pkgPath);
}
