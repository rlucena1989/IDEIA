...
## 13. FRONTEIRAS � Type-Safe Client Gen, WebSocket Gateway & GraphQL Versioning

> **Prop�sito:** SDK com gera��o autom�tica de clientes tipados, gateway WebSocket real-time e versionamento GraphQL
> **Frontier References:** "OpenAPI-Generator v7" (2025), "GraphQL Federation v2" � Apollo (2024), "WebSocket API Gateway Patterns" � AWS (2025)

### 13.1 TypeSafeClientGenerator � Gera��o de SDK Cliente Totalmente Tipado

Gera SDK client TypeScript/OpenAPI com tipos 100% inferidos do schema:
...
class TypeSafeClientGenerator {
  generate(config: GeneratorConfig): GeneratedSDK {
    const endpoints = this.extractEndpoints(config.spec);
    const types = this.generateTypes(endpoints, config.spec, config.language);
    const client = this.generateClientCode(endpoints, config);
    const hooks = config.includeReactHooks ? this.generateReactHooks(endpoints) : '';
    const tests = config.generateTests ? this.generateTests(endpoints) : '';
    return {
      files: [
        { path: 'types.ts', content: types },
        { path: 'client.ts', content: client },
        ...(hooks ? [{ path: 'hooks.ts', content: hooks }] : []),
        ...(tests ? [{ path: 'client.test.ts', content: tests }] : []),
      ],
      stats: { endpointCount: endpoints.length, typeCount: types.split('\n').filter(l => l.includes('interface') || l.includes('type ')).length },
    };
  }

  private extractEndpoints(spec: OpenAPIObject): EndpointDef[] {
    ...
    for (const [path, methods] of Object.entries(spec.paths || {})) {
      for (const [method, def] of Object.entries(methods as any)) {
        ...
        endpoints.push({
          ...
          operationId: (def as any).operationId || ${method},
          params: this.extractParams(path, def as any),
          requestBody: this.extractRequestBody(def as any),
          response: this.extractResponse(def as any),
  ...
  private generateTypes(_endpoints: EndpointDef[], spec: OpenAPIObject, language: string): string {
    const schemas = (spec.components?.schemas || {}) as Record<string, any>;
    let output = '// Auto-generated by IDEIA TypeSafeClientGenerator\n\n';
    for (const [name, schema] of Object.entries(schemas)) {
      output += this.schemaToType(name, schema as any, language) + '\n\n';
    }
    return output;
  }

  private schemaToType(name: string, schema: any, lang: string): string {
    if (lang === 'typescript') {
      const props = Object.entries(schema.properties || {}).map(([key, prop]: [string, any]) => '  ' + key + (schema.required?.includes(key) ? '' : '?') + ': ' + this.tsTypeFromSchema(prop) + ';');
      return 'export interface ' + name + ' {\n' + props.join('\n') + '\n}';
    }
    return '';
  }

  private tsTypeFromSchema(prop: any): string {
    if (prop.) return prop..split('/').pop() || 'unknown';
    if (prop.type === 'array') return this.tsTypeFromSchema(prop.items || { type: 'unknown' }) + '[]';
    const m: Record<string, string> = { string: 'string', integer: 'number', number: 'number', boolean: 'boolean', object: 'Record<string, unknown>' };
    return m[prop.type] || 'unknown';
  }

  private generateClientCode(endpoints: EndpointDef[], config: GeneratorConfig): string {
    if (config.language !== 'typescript') return '';
    let code = 'import type { ' + endpoints.map(ep => ep.operationId + 'Params, ' + ep.operationId + 'Response').join(', ') + " } from './types';\n\nexport class ApiClient {\n  constructor(private baseUrl: string, private headers?: Record<string, string>) {}\n\n";
    for (const ep of endpoints) {
      let path = ep.path;
      for (const p of ep.params.filter(p => p.in === 'path')) path = path.replace('{' + p.name + '}', '');
      code += '  async ' + ep.operationId + '(params: ' + ep.operationId + 'Params): Promise<' + ep.operationId + 'Response> {\n    const res = await fetch(${this.baseUrl}' + path + ', { method: \'' + ep.method + '\', headers: { \'Content-Type\': \'application/json\', ...this.headers }' + (ep.requestBody ? ',\n    body: JSON.stringify(params.body)' : '') + ' });\n    if (!res.ok) throw new Error(API );\n    return res.json();\n  }\n\n';
    }
    return code + '}\n';
  }

  private extractParams(path: string, def: any): Array<{ name: string; in: string; required: boolean; schema: any }> {
    const params = [...(def.parameters || [])];
    for (const pp of (path.match(/\{(\w+)\}/g) || []).map(p => p.slice(1, -1))) { if (!params.find((p: any) => p.name === pp)) params.push({ name: pp, in: 'path', required: true, schema: { type: 'string' } }); }
    return params;
  }
  private extractRequestBody(def: any): any { return def.requestBody?.content?.['application/json']?.schema; }
  private extractResponse(def: any): any { return def.responses?.['200']?.content?.['application/json']?.schema || def.responses?.['201']?.content?.['application/json']?.schema; }
}

interface EndpointDef { method: string; path: string; operationId: string; params: Array<{ name: string; in: string; required: boolean; schema: any }>; requestBody?: any; response?: any; summary: string; }
interface GeneratedSDK { files: Array<{ path: string; content: string }>; stats: { endpointCount: number; typeCount: number }; }
`

### 13.2 WebSocketAPIGateway � Gateway Bidirecional com Subscri��es

Gateway WebSocket com topics, subscri��es, reconnect e backpressure:

`	ypescript
class WebSocketAPIGateway {
  private connections = new Map<string, { ws: any; userId: string; subscriptions: Set<string>; lastHeartbeat: number }>();
  private subscriptions = new Map<string, Map<string, { id: string; userId: string; topic: string; filter?: any; createdAt: string }>>();

  constructor(private topicAuthorizer: (userId: string, topic: string) => boolean) {}

  handleConnection(ws: any, userId: string): string {
    const connId = crypto.randomUUID();
    this.connections.set(connId, { ws, userId, subscriptions: new Set(), lastHeartbeat: Date.now() });
  ...
  async publish(topic: string, event: any): Promise<void> {
    ...
    for (const [, sub] of subs) {
      ...
      const conn = [...this.connections.values()].find(c => c.userId === sub.userId);
      if (conn?.ws.readyState === 1) { try { conn.ws.send(JSON.stringify({ type: 'event', topic, subId: sub.id, data: event })); } catch {} }
    }
  }
  ...
  private handleDisconnect(connId: string): void {
    ...
    if (!conn) return;
    for (const subId of conn.subscriptions) { for (const [, subs] of this.subscriptions) subs.delete(subId); }
    this.connections.delete(connId);
  }
}
`

### 13.3 GraphQLAPIVersioner � Versionamento com GraphQL Federation

Evolu��o backward-compatible usando schema diff e versionamento autom�tico:

`	ypescript
class GraphQLAPIVersioner {
  private schemas = new Map<string, string>();
  private currentVersion = '1.0';

  registerSubgraph(name: string, sdl: string): void {
    const old = this.schemas.get(name);
    if (old) {
      const breaking = this.detectBreaking(old, sdl);
      if (breaking > 0) this.currentVersion = (parseInt(this.currentVersion) + 1) + '.0';
    }
    this.schemas.set(name, sdl);
  }

  private detectBreaking(oldSdl: string, newSdl: string): number {
    let breaking = 0;
    const oldTypes = this.parseTypes(oldSdl);
    const newTypes = this.parseTypes(newSdl);
    for (const [name, nt] of newTypes) {
      ...
      for (const of_ of ot.fields) { if (!nt.fields.find(f => f.name === of_.name)) breaking++; }
    }
    return breaking;
  }

  private parseTypes(sdl: string): Map<string, { name: string; fields: Array<{ name: string; type: string; nullable: boolean }> }> {
    const types = new Map();
    const re = /type\s+(\w+)\s*\{([^}]+)\}/g;
    let m;
    while ((m = re.exec(sdl)) !== null) {
      const fields = m[2].split('\n').filter(Boolean).map(l => { const p = l.trim().split(/:\s*/); return { name: p[0]?.split(' ')[0] || '', type: p[1]?.replace(/[!\[\]]/g, '') || 'String', nullable: !p[1]?.includes('!') }; });
...
- "OpenAPI Generator v7 � Multi-language SDK Generation" (2025)
- Apollo "GraphQL Federation v2 � Composition & Versioning" (2024)
- AWS "WebSocket API Gateway Patterns for Real-time Apps" (2025)
- "TypeScript 5.5 � Generated Types from API Schemas" (2025)
- "Schema Evolution in GraphQL Federation" � Apollo (2025)
