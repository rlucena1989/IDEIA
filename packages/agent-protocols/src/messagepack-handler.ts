...
const logger = createLogger('messagepack-handler');

export class MessagePackHandler {
  private _encoder = new TextEncoder();
  private _decoder = new TextDecoder();

  serialize(msg: Envelope): Uint8Array {
    const json = this._encoder.encode(JSON.stringify(msg));
    const header = this._encoder.encode('mpac');
  ...
  detectFormat(data: Uint8Array): SerializationFormat {
    const header = this._decoder.decode(data.slice(0, 4));
    if (header === 'mpac') return SerializationFormat.MESSAGEPACK;
    return SerializationFormat.JSON;
  }

  pack(value: unknown): Uint8Array {
    return this._encoder.encode(JSON.stringify(value));
  }

  unpack<T>(data: Uint8Array): T {
    return JSON.parse(this._decoder.decode(data)) as T;
  }
