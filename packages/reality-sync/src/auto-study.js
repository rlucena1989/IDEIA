...
exports.AutoStudyGenerator = void 0;
exports.createAutoStudyGenerator = createAutoStudyGenerator;
...
class AutoStudyGenerator {
    ...
    async generateStudy(title, topic, technologies) {
        const techResearch = await Promise.all(technologies.map(tech => this.researchTechnology(tech)));
        ...
        const scores = techResearch.filter((t) => t !== null);
    ...
    async researchTechnology(name) {
        ...
        const overall = Math.round((valueScore * 3 + differentiationScore * 2 + synergyScore * 2 + costBenefitScore * 2 + maturityScore * 1) / 10 * 10) / 10;
        let recommendation = 'INVESTIGAR';
        if (overall >= 3.5)
            recommendation = 'FAZER';
        else if (overall >= 2.5)
            recommendation = 'AGENDAR';
        else if (overall < 2)
            recommendation = 'NAO_FAZER';
        return { technology: name, overall, dimensions, recommendation };
    }
    async createStudyDocument(study) {
    ...
    estimateViability(technology) {
        return this.researchTechnology(technology);
    }
    getOverallRecommendation(scores) {
    ...
    generateViabilityMatrix(scores) {
    ...
    generateRiskSection(_topic, _technologies) {
...
exports.AutoStudyGenerator = AutoStudyGenerator;
function createAutoStudyGenerator(workspaceRoot) {
    return new AutoStudyGenerator(workspaceRoot);
}
//# sourceMappingURL=auto-study.js.map