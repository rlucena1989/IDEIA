...
export class TechRadarAPI {
  ...
  async scanGithubTrending(language: string, _since: 'daily' | 'weekly' | 'monthly' = 'weekly'): Promise<TechItem[]> {
    ...
    try {
      ...
      if (!res.ok) return this.getFallbackGithub(language);

      const data = await res.json() as { items?: Array<{ name: string; full_name: string; description: string; stargazers_count: number; html_url: string; pushed_at: string; language: string }> };
      if (!data.items) return this.getFallbackGithub(language);
  ...
  async scanNpmDownloads(packageName: string): Promise<{ name: string; downloads: number; score: number }> {
    try {
      const url = `https://api.npmjs.org/downloads/point/last-month/${encodeURIComponent(packageName)}`;
      const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
      if (!res.ok) return { name: packageName, downloads: 0, score: 0 };
  ...
  async scanArxivPapers(topic: string, maxResults: number = 10): Promise<TechItem[]> {
    const results: TechItem[] = [];
    try {
      ...
      if (!res.ok) return this.getFallbackArxiv(topic);
  ...
  evaluateTechnology(name: string, data: { stars: number; downloads: number; recencyDays: number }): TechEvaluation {
  ...
  getRecommendations(minScore: number = 3): TechRecommendation[] {
    return this.items
    ...
    const autoScan = async () => {
      try {
        ...
        for (const lang of languages) {
          ...
          for (const r of results) this.addItem(r);
        }
  ...
  getLatestResults(limit: number = 20): TechItem[] {
    return this.items.slice(-limit);
  }

  private recordScan(source: string, query: string): void {
  ...
  private getFallbackGithub(language: string): TechItem[] {
    const fallbacks: Record<string, TechItem[]> = {
      typescript: [
  ...
  private getFallbackArxiv(topic: string): TechItem[] {
    return [
...
export class TechRadar {
  ...
  async scanGitHub(language: string, limit = 10): Promise<TechItem[]> {
    const api = new TechRadarAPI();
    const results = await api.scanGithubTrending(language, 'weekly');
    return results.slice(0, limit);
  }

  async scanNpm(keyword: string, limit = 10): Promise<TechItem[]> {
    const results: TechItem[] = [];
    try {
      const url = `https://registry.npmjs.org/-/v1/search?text=${encodeURIComponent(keyword)}&size=${limit}`;
      const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
      if (!res.ok) return this.getFallbackNpm(keyword);

      const data = await res.json() as { objects?: Array<{ package: { name: string; description: string; version: string; keywords?: string[] }; score: { detail: { popularity: number; quality: number; maintenance: number } } }> };
      if (!data.objects) return this.getFallbackNpm(keyword);
  ...
  async scanArxiv(topic: string, limit = 10): Promise<TechItem[]> {
    const api = new TechRadarAPI();
  ...
  getRecommendations(minScore = 3): TechItem[] {
    return this.items
      .filter(i => i.score >= minScore)
      .sort((a, b) => b.score - a.score);
  }
  ...
  private getFallbackNpm(keyword: string): TechItem[] {
    return [
...
export function createTechRadar(): TechRadar {
  return new TechRadar();
}

export function createTechRadarAPI(): TechRadarAPI {
