...
const logger = (0, logger_1.createLogger)('config-manager');
class ConfigManager {
    static instance;
    secrets = new Map();
    loaded = false;
    ...
    loadEnvFile(filePath) {
        const resolved = (0, path_1.resolve)(process.cwd(), filePath);
    ...
    get(key, fallback) {
        ...
        if (schema) {
            return this.cast(value, schema.type);
        }
        return value;
    }
    getSecret(key) {
    ...
    getAll(includeSecrets = false) {
        const result = {};
        for (const [key, schema] of Object.entries(this.GLOBAL_SCHEMA)) {
            if (schema.sensitive && !includeSecrets)
                continue;
