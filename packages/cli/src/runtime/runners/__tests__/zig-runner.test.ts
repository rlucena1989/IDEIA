...
import { ZigRunner } from '../zig-runner';
...
describe('ZigRunner', () => {
  let runner: ZigRunner;

  beforeEach(() => {
    runner = new ZigRunner();
  });

  describe('configuration', () => {
    it('has correct language', () => {
      expect(runner.language).toBe(LanguageId.Zig);
    });

    it('has correct name', () => {
      expect(runner.name).toBe('Zig Runner');
    });

    it('has correct aliases', () => {
      expect(runner.aliases).toEqual(['zig', 'z']);
    });

    it('has correct markers', () => {
      expect((runner as any).config.markers).toEqual(['build.zig']);
    });
  });
  ...
  describe('commands', () => {
    it('has init command with correct argv', () => {
      ...
      expect(init!.label).toBe('Build (init)');
    });
    ...
    it('has compile command', () => {
      ...
      expect(compile!.label).toBe('Compile exe');
    });
  ...
  describe('detect', () => {
    ...
    it('returns true when marker exists', () => {
      jest.spyOn(fs, 'readdirSync').mockReturnValue([mockDirent('build.zig')]);
