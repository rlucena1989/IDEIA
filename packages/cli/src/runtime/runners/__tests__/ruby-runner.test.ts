...
describe('RubyRunner', () => {
  let runner: RubyRunner;

  beforeEach(() => {
    runner = new RubyRunner();
  });

  describe('configuration', () => {
    it('has correct language', () => {
      expect(runner.language).toBe(LanguageId.Ruby);
    });

    it('has correct name', () => {
      expect(runner.name).toBe('Ruby Runner');
    });

    it('has correct aliases', () => {
      expect(runner.aliases).toEqual(['ruby', 'rb']);
    });

    it('has correct markers', () => {
      expect(runner['config'].markers).toEqual(['Gemfile']);
    });
  });
  ...
  describe('commands', () => {
    it('has init command with correct argv', () => {
      ...
      expect(init!.description).toBe('Instala dependências (bundle install)');
    });
    ...
    it('has build command with correct argv', () => {
      ...
      expect(build!.label).toBe('Build gem');
    });
  ...
  describe('detect', () => {
    ...
    it('returns true when marker exists', () => {
      jest.spyOn(fs, 'readdirSync').mockReturnValue([mockDirent('Gemfile')] as any);
