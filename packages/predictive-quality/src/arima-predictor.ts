import { Forecast, ModelMetrics } from './types';
import { createLogger } from '@ideia/logger';
const logger = createLogger('arima-predictor');
...
export class ARIMAPredictor {
  async forecast(values: number[], horizon: number): Promise<Forecast> {
    ...
    const phi1 = this._autocorrelation(diff, 1);
    const residuals = diff.slice(1).map((v, i) => v - phi1 * diff[i]);
    const theta1 = this._autocorrelation(residuals, 1);
  ...
  async evaluate(values: number[], testSize: number): Promise<ModelMetrics> {
    ...
    const mape = test.reduce((sum, actual, i) => sum + Math.abs((actual - (forecast.predictions[i] ?? 0)) / Math.max(actual, 0.001)), 0) / testSize;
    const rmse = Math.sqrt(test.reduce((sum, actual, i) => sum + (actual - (forecast.predictions[i] ?? 0)) ** 2, 0) / testSize);
    return { mape, rmse, forecasts: forecast.predictions };
  }

  private _autocorrelation(values: number[], lag: number): number {
    const mean = values.reduce((s, v) => s + v, 0) / values.length;
    const num = values.slice(lag).reduce((s, v, i) => s + (v - mean) * ((values[i] ?? 0) - mean), 0);
