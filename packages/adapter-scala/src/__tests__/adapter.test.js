...
describe('adapter-scala', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapter-scala-test-'));
  });
  ...
  it('should not detect non-existent path', () => {
    expect(detect(path.join(os.tmpdir(), 'non-existent-scala-999'))).toBe(false);
  });
