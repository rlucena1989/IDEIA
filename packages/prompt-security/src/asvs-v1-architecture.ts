...
const logger = createLogger('asvs-v1-architecture');

const CATEGORY: AsvsCategory = 'V1';
...
export function checkV1_1_1(rootDir: string): AsvsCheck {
  ...
  if (sdlcPolicies.length > 0) evidence.push(`SDLC policies: ${sdlcPolicies.slice(0, 3).join(', ')}`);
  if (docsPatterns.length > 0) evidence.push(`Security docs: ${docsPatterns.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No secure development lifecycle evidence found');
  return { id: '1.1.1', name: 'Verify secure development lifecycle exists', category: CATEGORY, level: 1, passed, evidence: evidence.join(' | ') };
}

export function checkV1_1_2(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (threatModels.length > 0) evidence.push(`Threat models: ${threatModels.slice(0, 3).join(', ')}`);
  if (securityReviews.length > 0) evidence.push(`Security reviews: ${securityReviews.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No threat modeling for critical components detected');
...
export function checkV1_2_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (archDocs.length > 0) evidence.push(`Architecture docs: ${archDocs.slice(0, 3).join(', ')}`);
  if (securityArch.length > 0) evidence.push(`Security architecture: ${securityArch.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No security architecture documentation found');
...
export function checkV1_4_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (separationPatterns.length > 0) evidence.push(`Component separation: ${separationPatterns.slice(0, 3).join(', ')}`);
  if (nginxConfigs.length > 0) evidence.push(`Gateway/proxy: ${nginxConfigs.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No tier/component separation detected');
...
export function checkV1_5_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (securityReqs.length > 0) evidence.push(`Security requirements: ${securityReqs.slice(0, 4).join(', ')}`);
  if (policyFiles.length > 0) evidence.push(`Specifications: ${policyFiles.slice(0, 2).join(', ')}`);
  if (!passed) evidence.push('No security requirements for components found');
...
export function checkV1_6_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (eslintConfig.length > 0) evidence.push(`Linter configs: ${eslintConfig.slice(0, 3).join(', ')}`);
  if (securityPlugins.length > 0) evidence.push(`Security linter plugins: ${securityPlugins.join(', ')}`);
  if (codingStandards.length > 0) evidence.push(`Coding standard tools: ${codingStandards.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No secure coding standards detected');
  return { id: '1.6.1', name: 'Verify secure coding standards exist', category: CATEGORY, level: 1, passed, evidence: evidence.join(' | ') };
}

export function checkV1_7_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (changeMgmt.length > 0) evidence.push(`Change management: ${changeMgmt.slice(0, 3).join(', ')}`);
  if (ciSecurity.length > 0) evidence.push(`CI security checks: ${ciSecurity.slice(0, 3).join(', ')}`);
  if (!passed) evidence.push('No security change impact analysis found');
...
export function checkV1_8_1(rootDir: string): AsvsCheck {
  ...
  const evidence: string[] = [];
  if (componentInventory.length > 0) evidence.push(`Component inventory: ${componentInventory.slice(0, 3).join(', ')}`);
  if (servicesPatterns.length > 0) evidence.push(`Service modules: ${servicesPatterns.slice(0, 4).join(', ')}`);
  if (!passed) evidence.push('No component inventory or service registry detected');
...
export function runAllV1Checks(rootDir: string): AsvsCheck[] {
  return [
    checkV1_1_1(rootDir),
    checkV1_1_2(rootDir),
    checkV1_2_1(rootDir),
