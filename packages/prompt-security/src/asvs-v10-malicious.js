...
exports.checkV10_1_1 = checkV10_1_1;
exports.checkV10_2_1 = checkV10_2_1;
exports.checkV10_3_1 = checkV10_3_1;
exports.checkV10_4_1 = checkV10_4_1;
exports.checkV10_5_1 = checkV10_5_1;
exports.checkV10_6_1 = checkV10_6_1;
exports.checkV10_7_1 = checkV10_7_1;
exports.runAllV10Checks = runAllV10Checks;
const node_fs_1 = __importDefault(require("node:fs"));
const logger_1 = require("@ideia/logger");
const node_path_1 = __importDefault(require("node:path"));
const logger = (0, logger_1.createLogger)('asvs-v10-malicious');
const CATEGORY = 'V10';
...
function checkV10_1_1(rootDir) {
    ...
    if (integrityPatterns.length > 0)
        evidence.push(`Integrity checks: ${integrityPatterns.slice(0, 3).join(', ')}`);
    if (sriInHtml.length > 0)
        evidence.push(`SRI found: ${sriInHtml.slice(0, 3).join(', ')}`);
    if (buildScripts.length > 0)
        evidence.push(`Build process: ${buildScripts.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No code integrity or SRI checks detected');
    return {
        id: '10.1.1',
...
function checkV10_2_1(rootDir) {
    ...
    const evidence = [];
    if (codeReviewPatterns.length > 0)
        evidence.push(`Code review processes: ${codeReviewPatterns.slice(0, 3).join(', ')}`);
    if (securityReviewPatterns.length > 0)
        evidence.push(`Security analysis: ${securityReviewPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No code review process for security detected');
...
function checkV10_3_1(rootDir) {
    ...
    const evidence = [];
    if (antiTamperPatterns.length > 0)
        evidence.push(`Anti-tamper mechanisms: ${antiTamperPatterns.slice(0, 3).join(', ')}`);
    if (lockFiles.length > 0)
        evidence.push(`Dependency lock files: ${lockFiles.slice(0, 3).join(', ')}`);
    if (dependencyLock.length > 0)
        evidence.push(`Lock file references: ${dependencyLock.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No anti-tampering or dependency lock verification detected');
    return {
        id: '10.3.1',
...
function checkV10_4_1(rootDir) {
    ...
    const evidence = [];
    if (ciPipeline.length > 0)
        evidence.push(`CI/CD pipeline: ${ciPipeline.slice(0, 3).join(', ')}`);
    if (ciIntegrity.length > 0)
        evidence.push(`Pipeline integrity: ${ciIntegrity.slice(0, 3).join(', ')}`);
    if (!passed && ciPipeline.length > 0)
        evidence.push('CI/CD detected without pipeline integrity verification');
    if (!passed && ciPipeline.length === 0)
        evidence.push('No CI/CD pipeline integrity detected');
    return { id: '10.4.1', name: 'Verify CI/CD pipeline integrity', category: CATEGORY, level: 2, passed, evidence: evidence.join(' | ') };
}
function checkV10_5_1(rootDir) {
    ...
    const evidence = [];
    if (vulnScanDeps.length > 0)
        evidence.push(`Vulnerability scanners: ${vulnScanDeps.join(', ')}`);
    if (vulnScanPatterns.length > 0)
        evidence.push(`Vulnerability patterns: ${vulnScanPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No dependency vulnerability scanning detected');
    return { id: '10.5.1', name: 'Verify dependency vulnerability scanning', category: CATEGORY, level: 2, passed, evidence: evidence.join(' | ') };
}
function checkV10_6_1(rootDir) {
    ...
    const evidence = [];
    if (scaPatterns.length > 0)
        evidence.push(`SCA patterns: ${scaPatterns.slice(0, 3).join(', ')}`);
    if (sbomFiles.length > 0)
        evidence.push(`SBOM files: ${sbomFiles.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No software composition analysis detected');
    return { id: '10.6.1', name: 'Verify software composition analysis (SCA)', category: CATEGORY, level: 3, passed, evidence: evidence.join(' | ') };
}
function checkV10_7_1(rootDir) {
    ...
    const evidence = [];
    if (raspPatterns.length > 0)
        evidence.push(`RASP patterns: ${raspPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No runtime application self-protection (RASP) detected');
    return { id: '10.7.1', name: 'Verify runtime application self-protection', category: CATEGORY, level: 3, passed, evidence: evidence.join(' | ') };
}
function runAllV10Checks(rootDir) {
    return [
...
//# sourceMappingURL=asvs-v10-malicious.js.map