...
exports.checkV2_1_1 = checkV2_1_1;
exports.checkV2_1_2 = checkV2_1_2;
exports.checkV2_2_1 = checkV2_2_1;
exports.checkV2_3_1 = checkV2_3_1;
exports.checkV2_4_1 = checkV2_4_1;
exports.checkV2_5_1 = checkV2_5_1;
exports.checkV2_6_1 = checkV2_6_1;
exports.checkV2_7_1 = checkV2_7_1;
exports.checkV2_8_1 = checkV2_8_1;
exports.checkV2_9_1 = checkV2_9_1;
exports.checkV2_10_1 = checkV2_10_1;
exports.checkV2_11_1 = checkV2_11_1;
exports.runAllV2Checks = runAllV2Checks;
const node_fs_1 = __importDefault(require("node:fs"));
const logger_1 = require("@ideia/logger");
const node_path_1 = __importDefault(require("node:path"));
const logger = (0, logger_1.createLogger)('asvs-v2-auth');
const CATEGORY = 'V2';
...
function checkV2_1_1(rootDir) {
    ...
    if (authDeps.length > 0)
        evidence.push(`Auth dependencies found: ${authDeps.slice(0, 5).join(', ')}`);
    if (authPatterns.length > 0)
        evidence.push(`Auth patterns in code: ${authPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No authentication libraries or patterns detected');
    return {
        id: '2.1.1',
...
function checkV2_1_2(rootDir) {
    ...
    const evidence = [];
    if (bcryptDeps.length > 0)
        evidence.push(`Password hashing: ${bcryptDeps.slice(0, 3).join(', ')}`);
    if (passwordPatterns.length > 0)
        evidence.push(`Password rules: ${passwordPatterns.slice(0, 2).join(', ')}`);
    if (hasZodPassword.length > 0)
        evidence.push(`Zod password validation: ${hasZodPassword.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No password strength requirements detected');
    return {
        id: '2.1.2',
...
function checkV2_2_1(rootDir) {
    ...
    const evidence = [];
    if (strongHashing.length > 0)
        evidence.push(`Strong hashing libs: ${strongHashing.join(', ')}`);
    if (hashPatterns.length > 0)
        evidence.push(`Hash verification: ${hashPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No approved credential storage encryption detected');
...
function checkV2_3_1(rootDir) {
    ...
    const evidence = [];
    if (mfaAdminPatterns.length > 0)
        evidence.push(`Admin MFA patterns: ${mfaAdminPatterns.slice(0, 3).join(', ')}`);
    if (mfaEnforcePatterns.length > 0)
        evidence.push(`MFA enforcement: ${mfaEnforcePatterns.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No MFA enforcement for admin accounts detected');
    return { id: '2.3.1', name: 'Verify MFA is enforced for admin accounts', category: CATEGORY, level: 2, passed, evidence: evidence.join(' | ') };
}
function checkV2_4_1(rootDir) {
    ...
    const evidence = [];
    if (credStrengthPatterns.length > 0)
        evidence.push(`Credential strength config: ${credStrengthPatterns.slice(0, 3).join(', ')}`);
    if (credConfigFiles.length > 0)
        evidence.push(`Config files: ${credConfigFiles.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No credential strength configuration detected');
...
function checkV2_5_1(rootDir) {
    ...
    const evidence = [];
    if (apiKeyDeps.length > 0)
        evidence.push(`API key deps: ${apiKeyDeps.join(', ')}`);
    if (apiKeyPatterns.length > 0)
        evidence.push(`API key patterns: ${apiKeyPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No API key authentication detected');
    return {
        id: '2.5.1',
...
function checkV2_6_1(rootDir) {
    ...
    const evidence = [];
    if (noncePatterns.length > 0)
        evidence.push(`Nonce/timestamp protection: ${noncePatterns.slice(0, 3).join(', ')}`);
    if (jwtClaims.length > 0)
        evidence.push(`JWT expiry claims: ${jwtClaims.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No replay attack protection detected');
...
function checkV2_7_1(rootDir) {
    ...
    const evidence = [];
    if (hsmPatterns.length > 0)
        evidence.push(`HSM/secure storage: ${hsmPatterns.slice(0, 3).join(', ')}`);
    if (hsmDeps.length > 0)
        evidence.push(`KMS integrations: ${hsmDeps.join(', ')}`);
    if (!passed)
        evidence.push('No hardware-backed key storage detected');
    return { id: '2.7.1', name: 'Verify hardware-backed key storage (HSM/TPM)', category: CATEGORY, level: 3, passed, evidence: evidence.join(' | ') };
}
function checkV2_8_1(rootDir) {
    ...
    if (recoveryPatterns.length > 0)
        evidence.push(`Recovery patterns: ${recoveryPatterns.slice(0, 3).join(', ')}`);
    if (recoveryDeps.length > 0)
        evidence.push(`Email deps for recovery: ${recoveryDeps.join(', ')}`);
    if (!passed)
        evidence.push('No credential recovery process detected');
    return {
        id: '2.8.1',
...
function checkV2_9_1(rootDir) {
    ...
    if (invalidationPatterns.length > 0)
        evidence.push(`Session invalidation on password change: ${invalidationPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No session invalidation on password change detected');
...
function checkV2_10_1(rootDir) {
    ...
    const evidence = [];
    if (mfaDeps.length > 0)
        evidence.push(`MFA deps: ${mfaDeps.join(', ')}`);
    if (mfaPatterns.length > 0)
        evidence.push(`MFA patterns: ${mfaPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No MFA configuration detected');
    return {
        id: '2.10.1',
...
function checkV2_11_1(rootDir) {
    ...
    const evidence = [];
    if (lockoutPatterns.length > 0)
        evidence.push(`Lockout patterns: ${lockoutPatterns.slice(0, 3).join(', ')}`);
    if (lockoutConfigs.length > 0)
        evidence.push(`Lockout config: ${lockoutConfigs.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No account lockout after brute force attempts detected');
...
function runAllV2Checks(rootDir) {
    return [
        checkV2_1_1(rootDir),
        checkV2_1_2(rootDir),
        checkV2_2_1(rootDir),
...
//# sourceMappingURL=asvs-v2-auth.js.map