import { SecurityRule, SecurityIssue, SeverityLevel, SanitizationAction } from './types';
import { createLogger } from '@ideia/logger';
const logger = createLogger('pii-output-validator');
...
const ENHANCED_PII_RULES: SecurityRule[] = [
  // Brazilian PII
...
const CONTEXT_HINTS: Record<string, RegExp[]> = {
...
export class PiiOutputValidator {
  private enhancedRules: SecurityRule[];
  ...
  validate(output: string): PiiConfidenceResult {
    ...
    const riskLevel: 'low' | 'medium' | 'high' | 'critical' = maxSeverityIdx >= 0
  ...
  private evaluateContext(output: string, matchPos: number, category: string, matchValue: string): PiiContextResult {
  ...
  private adjustSeverity(severity: SeverityLevel, confidence: number): SeverityLevel {
    if (confidence >= 0.9) return severity;
    if (confidence >= 0.7) return severity;
    if (confidence >= 0.5) {
...
export function createPiiOutputValidator(): PiiOutputValidator {
