...
exports.PromptSecurity = void 0;
exports.createPromptSecurity = createPromptSecurity;
const pii_output_validator_1 = require("./pii-output-validator");
const DEFAULT_RULES = [
...
const CODE_VULNERABILITY_RULES = [
...
class PromptSecurity {
    ...
    constructor(customRules) {
        ...
        this.piiValidator = new pii_output_validator_1.PiiOutputValidator();
    }
    scan(prompt) {
        const issues = [];
    ...
    validateGeneratedCode(code, options) {
        const issues = [];
        const opts = { checkSecrets: true, checkInjections: true, checkXSS: true, checkSQL: true, checkServiceSecrets: true, ...options };
        let activeRules = this.codeRules;
        if (!opts.checkSecrets)
            activeRules = activeRules.filter(r => r.category !== 'hardcoded-secret' && r.category !== 'service-secret');
        if (!opts.checkInjections)
            activeRules = activeRules.filter(r => r.category !== 'eval-usage' && r.category !== 'function-constructor' && r.category !== 'template-injection');
        if (!opts.checkXSS)
            activeRules = activeRules.filter(r => !r.category.startsWith('xss-') && r.category !== 'script-injection');
        if (!opts.checkSQL)
            activeRules = activeRules.filter(r => r.category !== 'sql-injection' && r.category !== 'sql-destructive');
        if (!opts.checkServiceSecrets)
            activeRules = activeRules.filter(r => r.category !== 'env-usage' && r.category !== 'hardcoded-host');
    ...
    getInjectionSuggestion(category) {
        const suggestions = {
    ...
    applySanitization(prompt, issues) {
...
exports.PromptSecurity = PromptSecurity;
function createPromptSecurity(customRules) {
