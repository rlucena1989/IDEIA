...
exports.checkV4_1_1 = checkV4_1_1;
exports.checkV4_1_2 = checkV4_1_2;
exports.checkV4_1_3 = checkV4_1_3;
exports.checkV4_2_1 = checkV4_2_1;
exports.checkV4_2_2 = checkV4_2_2;
exports.checkV4_3_1 = checkV4_3_1;
exports.checkV4_3_2 = checkV4_3_2;
exports.checkV4_3_3 = checkV4_3_3;
exports.runAllV4Checks = runAllV4Checks;
const node_fs_1 = __importDefault(require("node:fs"));
const logger_1 = require("@ideia/logger");
const node_path_1 = __importDefault(require("node:path"));
const logger = (0, logger_1.createLogger)('asvs-v4-access-control');
const CATEGORY = 'V4';
...
function checkV4_1_1(rootDir) {
    ...
    const evidence = [];
    if (authorizationDeps.length > 0)
        evidence.push(`AuthZ libraries: ${authorizationDeps.join(', ')}`);
    if (rolePatterns.length > 0)
        evidence.push(`Role/permission patterns: ${rolePatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No least privilege principle implementation detected');
...
function checkV4_1_2(rootDir) {
    ...
    if (denyPatterns.length > 0)
        evidence.push(`Default deny patterns: ${denyPatterns.slice(0, 3).join(', ')}`);
    if (guardPatterns.length > 0)
        evidence.push(`Access guards: ${guardPatterns.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No default deny access control detected');
    return { id: '4.1.2', name: 'Verify default deny access is enforced', category: CATEGORY, level: 1, passed, evidence: evidence.join(' | ') };
}
function checkV4_1_3(rootDir) {
    ...
    const evidence = [];
    if (privilegePatterns.length > 0)
        evidence.push(`Privilege separation: ${privilegePatterns.slice(0, 3).join(', ')}`);
    if (serviceAccountPatterns.length > 0)
        evidence.push(`Service accounts: ${serviceAccountPatterns.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No privilege separation detected');
    return { id: '4.1.3', name: 'Verify privilege separation is enforced', category: CATEGORY, level: 1, passed, evidence: evidence.join(' | ') };
}
function checkV4_2_1(rootDir) {
    ...
    if (sensitiveAccess.length > 0)
        evidence.push(`Sensitive data access controls: ${sensitiveAccess.slice(0, 3).join(', ')}`);
    if (dataLayerSecurity.length > 0)
        evidence.push(`Data layer security: ${dataLayerSecurity.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No access controls for sensitive data detected');
...
function checkV4_2_2(rootDir) {
    ...
    if (rbacPatterns.length > 0)
        evidence.push(`RBAC libraries: ${rbacPatterns.slice(0, 3).join(', ')}`);
    if (roleConfigs.length > 0)
        evidence.push(`Role definitions: ${roleConfigs.slice(0, 2).join(', ')}`);
    if (!passed)
        evidence.push('No role-based access controls detected');
...
function checkV4_3_1(rootDir) {
    ...
    if (adminControls.length > 0)
        evidence.push(`Admin controls: ${adminControls.slice(0, 3).join(', ')}`);
    if (adminEndpoints.length > 0)
        evidence.push(`Admin endpoints: ${adminEndpoints.slice(0, 3).join(', ')}`);
    if (adminDeps.length > 0)
        evidence.push(`Admin libraries: ${adminDeps.join(', ')}`);
    if (!passed)
        evidence.push('No administrative access controls detected');
...
function checkV4_3_2(rootDir) {
    ...
    if (endpointSecurity.length > 0)
        evidence.push(`Endpoint security: ${endpointSecurity.slice(0, 4).join(', ')}`);
    if (routeSecurity.length > 0)
        evidence.push(`Route guards: ${routeSecurity.slice(0, 3).join(', ')}`);
    if (!passed)
        evidence.push('No access control for all endpoints detected');
...
//# sourceMappingURL=asvs-v4-access-control.js.map