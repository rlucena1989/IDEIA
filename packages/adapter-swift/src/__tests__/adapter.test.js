...
describe('adapter-swift', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapter-swift-test-'));
  });
  ...
  it('should not detect non-existent path', () => {
    expect(detect(path.join(os.tmpdir(), 'non-existent-swift-999'))).toBe(false);
  });
