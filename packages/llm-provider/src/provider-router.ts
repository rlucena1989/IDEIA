...
export const UNIVERSAL_ENDPOINTS: Array<{ name: string; endpoint: string; defaultModel: string; envKey?: string }> = [
...
export const REASONING_ENDPOINTS: Array<{ name: string; endpoint: string; defaultModel: string; envKey: string }> = [
...
function resolveApiKey(envKey?: string): string | undefined {
...
function tryRegisterProvider(
  ...
  try {
    router.register(new OpenAIProvider({ endpoint, apiKey, defaultModel, providerName: name }));
    return true;
  } catch {
    return false;
  }
...
export function createDefaultRouter(): ProviderRouter {
  ...
  router.register(new OllamaProvider({ endpoint: 'http://localhost:11434', defaultModel: model || 'deepseek-coder' }));
  ...
  if (customEndpoint && !customEndpoint.includes('localhost') && !customEndpoint.includes('127.0.0.1')) {
    try {
      const primary = createProvider({ endpoint: customEndpoint, apiKey: masterKey, defaultModel: model });
  ...
  if (reasoningEnabled) {
    for (const ep of REASONING_ENDPOINTS) {
      const key = resolveApiKey(ep.envKey);
      if (key) {
        try {
          if (ep.name === 'openai-reasoning') router.register(new OpenAIReasoningProvider({ endpoint: ep.endpoint, apiKey: key, defaultModel: ep.defaultModel }));
          if (ep.name === 'deepseek-reasoning') router.register(new DeepSeekReasoningProvider({ endpoint: ep.endpoint, apiKey: key, defaultModel: ep.defaultModel }));
        } catch { /* skip */ }
      }
    }
  }
