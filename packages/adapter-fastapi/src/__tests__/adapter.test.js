...
describe('adapter-fastapi', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapter-fastapi-test-'));
  });
  ...
  it('should not detect non-existent path', () => {
    expect(detect(path.join(os.tmpdir(), 'non-existent-fastapi-999'))).toBe(false);
  });
