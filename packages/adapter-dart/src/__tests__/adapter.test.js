...
describe('adapter-dart', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'adapter-dart-test-'));
  });
  ...
  it('should not detect non-existent path', () => {
    expect(detect(path.join(os.tmpdir(), 'non-existent-dart-999'))).toBe(false);
  });
