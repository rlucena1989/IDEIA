...
import {
  NpsResponse, NpsReport, NpsScore,
  SusResponse, SusReport, SusScore,
  CesResponse, CesReport, CesScore,
  TimeToTaskRecord, TimeToTaskReport,
  PageLoadRecord, PageLoadReport,
  ErrorRecord, ErrorReport,
  SatisfactionRecord, SatisfactionReport, SatisfactionScore,
  AccessibilityRecord, AccessibilityReport,
  UxDashboard, UxAnalyticsConfig, PersistedMetrics,
  PerfMetric, PerfCategory, PerformanceReport,
} from './types';
const logger = createLogger('ux-metrics-collector');
...
export function normalizeNps(score: number): number {
...
export class UxMetricsCollector {
  ...
  private cesResponses: CesResponse[] = [];
  private timeToTaskRecords: TimeToTaskRecord[] = [];
  private pageLoads: PageLoadRecord[] = [];
  private errors: ErrorRecord[] = [];
  private satisfactionRecords: SatisfactionRecord[] = [];
  private a11yRecords: AccessibilityRecord[] = [];
  ...
  private saveSqlite(): void {
    if (!this.db) return;
    const db: BetterSqlite3.Database = this.db;
    ...
    const tx = db.transaction(() => {
      ...
      for (const r of this.npsResponses) insertNps.run(r.score, r.reason || null, r.timestamp, r.sessionId);
      for (const r of this.susResponses) insertSus.run(JSON.stringify(r.answers), r.score, r.timestamp, r.sessionId);
      for (const r of this.cesResponses) insertCes.run(r.taskId, r.taskLabel, r.score, r.durationMs, r.timestamp, r.sessionId);
      for (const r of this.timeToTaskRecords) insertTtt.run(r.taskId, r.taskLabel, r.startTime, r.endTime, r.durationMs, r.sessionId, r.completed ? 1 : 0);
      for (const r of this.pageLoads) insertPl.run(r.url, r.loadTimeMs, r.timestamp, r.sessionId);
      for (const r of this.errors) insertErr.run(r.errorType, r.message, r.count, r.timestamp, r.sessionId);
      for (const r of this.satisfactionRecords) insertSat.run(r.score, r.context || null, r.timestamp, r.sessionId);
      for (const r of this.a11yRecords) insertA11y.run(r.ruleId, r.score, r.violations, r.source, r.timestamp, r.sessionId);
    });
    tx();
  }

  private loadSqlite(): void {
    if (!this.db) return;
    const db: BetterSqlite3.Database = this.db;
    const limit = this.config.maxHistory;

    const npsRows = db.prepare('SELECT score, reason, timestamp, session_id FROM nps ORDER BY id DESC LIMIT ?').all(limit) as Array<{ score: number; reason: string | null; timestamp: string; session_id: string }>;
    for (const row of npsRows.reverse()) {
      this.npsResponses.push({ score: row.score as NpsScore, reason: row.reason || undefined, timestamp: row.timestamp, sessionId: row.session_id });
    }

    const susRows = db.prepare('SELECT answers, score, timestamp, session_id FROM sus ORDER BY id DESC LIMIT ?').all(limit) as Array<{ answers: string; score: number; timestamp: string; session_id: string }>;
    ...
    const cesRows = db.prepare('SELECT task_id, task_label, score, duration_ms, timestamp, session_id FROM ces ORDER BY id DESC LIMIT ?').all(limit) as Array<{ task_id: string; task_label: string; score: number; duration_ms: number; timestamp: string; session_id: string }>;
    for (const row of cesRows.reverse()) {
      this.cesResponses.push({ taskId: row.task_id, taskLabel: row.task_label, score: row.score as CesScore, durationMs: row.duration_ms, timestamp: row.timestamp, sessionId: row.session_id });
    }

    const tttRows = db.prepare('SELECT task_id, task_label, start_time, end_time, duration_ms, session_id, completed FROM time_to_task ORDER BY id DESC LIMIT ?').all(limit) as Array<{ task_id: string; task_label: string; start_time: string; end_time: string | null; duration_ms: number; session_id: string; completed: number }>;
    ...
    const plRows = db.prepare('SELECT url, load_time_ms, timestamp, session_id FROM page_loads ORDER BY id DESC LIMIT ?').all(limit) as Array<{ url: string; load_time_ms: number; timestamp: string; session_id: string }>;
    ...
    const errRows = db.prepare('SELECT error_type, message, count, timestamp, session_id FROM errors ORDER BY id DESC LIMIT ?').all(limit) as Array<{ error_type: string; message: string; count: number; timestamp: string; session_id: string }>;
    ...
    const satRows = db.prepare('SELECT score, context, timestamp, session_id FROM satisfaction ORDER BY id DESC LIMIT ?').all(limit) as Array<{ score: number; context: string | null; timestamp: string; session_id: string }>;
    for (const row of satRows.reverse()) {
      this.satisfactionRecords.push({ score: row.score as SatisfactionScore, context: row.context || undefined, timestamp: row.timestamp, sessionId: row.session_id });
    }

    const a11yRows = db.prepare('SELECT rule_id, score, violations, source, timestamp, session_id FROM a11y ORDER BY id DESC LIMIT ?').all(limit) as Array<{ rule_id: string; score: number; violations: number; source: string; timestamp: string; session_id: string }>;
  ...
  recordNps(score: NpsScore, reason?: string): NpsResponse {
    const resp: NpsResponse = { score, reason, timestamp: new Date().toISOString(), sessionId: this.config.sessionId };
    this.npsResponses.push(resp);
    if (this.npsResponses.length > this.config.maxHistory) this.npsResponses.shift();
    return resp;
  }
  ...
  recordSus(answers: number[]): SusResponse {
    ...
    const resp: SusResponse = { answers, score: score as SusScore, timestamp: new Date().toISOString(), sessionId: this.config.sessionId };
    this.susResponses.push(resp);
    if (this.susResponses.length > this.config.maxHistory) this.susResponses.shift();
    return resp;
  }
  ...
  recordCes(taskId: string, taskLabel: string, score: CesScore, durationMs: number): CesResponse {
    const resp: CesResponse = { taskId, taskLabel, score, timestamp: new Date().toISOString(), sessionId: this.config.sessionId, durationMs };
    this.cesResponses.push(resp);
    if (this.cesResponses.length > this.config.maxHistory) this.cesResponses.shift();
    return resp;
  }

  getCesReport(): CesReport {
    const byTaskType: CesReport['byTaskType'] = {};
    for (const r of this.cesResponses) {
      const key = r.taskLabel.split(':')[0] || 'general';
      if (!byTaskType[key]) byTaskType[key] = { count: 0, avgScore: 0, avgDurationMs: 0 };
  ...
  startTask(taskId: string, taskLabel: string): void {
    ...
    const record: TimeToTaskRecord = {
      taskId, taskLabel, startTime: new Date().toISOString(), endTime: '', durationMs: 0, sessionId: this.config.sessionId, completed: false,
    };
    if (existing >= 0) this.timeToTaskRecords[existing] = record;
    else this.timeToTaskRecords.push(record);
    if (this.timeToTaskRecords.length > this.config.maxHistory) this.timeToTaskRecords.shift();
  }
  ...
  getTimeToTaskReport(): TimeToTaskReport {
    ...
    const byTaskType: TimeToTaskReport['byTaskType'] = {};
    for (const r of completed) {
      const key = r.taskLabel.split(':')[0] || 'general';
      if (!byTaskType[key]) byTaskType[key] = { count: 0, avgDurationMs: 0 };
      const group = byTaskType[key];
      group.count++;
      group.avgDurationMs = Math.round((group.avgDurationMs * (group.count - 1) + r.durationMs) / group.count);
    }
  ...
  recordPageLoad(url: string, loadTimeMs: number): PageLoadRecord {
    const record: PageLoadRecord = { url, loadTimeMs, timestamp: new Date().toISOString(), sessionId: this.config.sessionId };
    this.pageLoads.push(record);
    if (this.pageLoads.length > this.config.maxHistory) this.pageLoads.shift();
    return record;
  }
  ...
  recordError(errorType: string, message: string): ErrorRecord {
    const record: ErrorRecord = { errorType, message, count: 1, timestamp: new Date().toISOString(), sessionId: this.config.sessionId };
    ...
    if (this.errors.length > this.config.maxHistory) this.errors.shift();
    return record;
  }
  ...
  recordSatisfaction(score: SatisfactionScore, context?: string): SatisfactionRecord {
    const record: SatisfactionRecord = { score, context, timestamp: new Date().toISOString(), sessionId: this.config.sessionId };
    this.satisfactionRecords.push(record);
    if (this.satisfactionRecords.length > this.config.maxHistory) this.satisfactionRecords.shift();
    return record;
  }
  ...
  recordAccessibility(ruleId: string, score: number, violations: number, source: string): AccessibilityRecord {
    ...
    if (this.a11yRecords.length > this.config.maxHistory) this.a11yRecords.shift();
    return record;
  }

  getAccessibilityReport(): AccessibilityReport {
    const scores = this.a11yRecords.map(r => r.score);
    const bySource: AccessibilityReport['bySource'] = {};
    for (const r of this.a11yRecords) {
      if (!bySource[r.source]) bySource[r.source] = { count: 0, avgScore: 0, totalViolations: 0 };
  ...
  getAll(): {
    nps: NpsResponse[]; sus: SusResponse[]; ces: CesResponse[];
    timeToTask: TimeToTaskRecord[]; pageLoads: PageLoadRecord[];
    errors: ErrorRecord[]; satisfaction: SatisfactionRecord[];
  ...
  private perfMetrics: PerfMetric[] = [];
  private optimisticOps: Map<string, number> = new Map();
  ...
  measurePageLoad(): number {
    ...
    if (this.perfMetrics.length > 100) this.perfMetrics.shift();
    return metric.duration;
  }

  async measureInteraction<T>(interactionName: string, fn: () => Promise<T>): Promise<T> {
    const start = performance?.now ? performance.now() : Date.now();
    try {
  ...
  getPerceivedPerformanceScore(): number {
    if (this.perfMetrics.length === 0) return 100;
...
export function createUxMetricsCollector(config?: Partial<UxAnalyticsConfig>): UxMetricsCollector {
