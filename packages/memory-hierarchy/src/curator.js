...
const logger = (0, logger_1.createLogger)('curator');
const DEFAULT_RULES = [
...
class MemoryCurator {
    ...
    global;
    config;
    constructor(working, project, institutional, global, config) {
    ...
    evaluatePromotions() {
        ...
        for (const entry of [...wmEntries, ...pmEntries]) {
            for (const rule of this.config.promotionRules) {
                if (entry.level !== rule.fromLevel)
                    continue;
                if (entry.accessCount < rule.minAccessCount)
                    continue;
                if (entry.confidence < rule.minConfidence)
                    continue;
                if (rule.requiredTags && !rule.requiredTags.some(t => entry.tags.includes(t)))
                    continue;
