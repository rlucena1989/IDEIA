...
const clientContent = [
  ...
  '    const res = await fetch(String.raw`${this.baseUrl}/api/v1/' + name + 's?page=${page}&perPage=${perPage}`)',
  '    if (!res.ok) throw new Error(String.raw`HTTP ${res.status}: ${await res.text()}`)',
  '    return res.json()',
  '  }',
  '',
  '  async getById(id: string) {',
  '    const res = await fetch(String.raw`${this.baseUrl}/api/v1/' + name + 's/${id}`)',
  '    if (!res.ok) throw new Error(String.raw`HTTP ${res.status}: ${await res.text()}`)',
  '    return res.json()',
  '  }',
  '}',
