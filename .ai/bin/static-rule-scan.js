...
if (lawsContent.includes('Cobertura de testes minima')) rules.push({ id: 'test-coverage', desc: 'Cobertura de testes minima: 20%', check: 'coverage' })
...
scanFiles(srcDir, (filePath, content) => {
  ...
  if (rules.find(r => r.id === 'app_error') && !rel.includes('.spec.')) {
    const hasThrow = /throw\s+new\s+(?!AppError)\w+/g.test(content)
    if (hasThrow) {
